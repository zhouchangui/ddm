---
name: ddm-slide-deck
description: Generates professional slide deck images from content. Creates outlines with style instructions, then generates individual slide images. Use when user asks to "create slides", "make a presentation", "generate deck", "slide deck", or "PPT". Uses ddm-image-gen as the fixed image generation backend.
version: 0.1.0
---
...
**Prompt file requirement (hard)**: write each image's full, final prompt to a standalone file under `prompts/` (naming: `NN-slide-[slug].md`) BEFORE invoking ddm-image-gen. The file is the reproducibility record.
...
At generation time, verify files exist. If `usage: direct` and the backend accepts refs, pass the file on every slide. Otherwise embed extracted `style`/`palette` traits in the prompt text.
...
| Path | Scope |
|------|-------|
| `.ddm-skills/ddm-slide-deck/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/ddm-skills/ddm-slide-deck/EXTEND.md` | XDG |
| `$HOME/.ddm-skills/ddm-slide-deck/EXTEND.md` | User home |
...
2. Confirm every `prompts/NN-slide-{slug}.md` exists (hard requirement; prompt files are the reproducibility record).
