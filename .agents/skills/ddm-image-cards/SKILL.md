...
| Path | Scope |
|------|-------|
| `.ddm-skills/ddm-image-cards/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/ddm-skills/ddm-image-cards/EXTEND.md` | XDG |
| `$HOME/.ddm-skills/ddm-image-cards/EXTEND.md` | User home |
...
**Backend**: use **ddm-image-gen** per the `## Image Generation Tools` section at the top. Prompt files MUST exist before invoking the backend.
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure ddm-image-cards preferences" / "重新配置"). The next run re-triggers first-time setup.
